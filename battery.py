...
# begin_test_date    = pd.to_datetime('17/9/2024 23:00', dayfirst=True) # Start test date, if not provided 'years_test' is used. Format: "%d/%m/%Y %H:%M"
# end_test_date      = pd.to_datetime('11/11/2024 19:15', dayfirst=True) # End test date, if not provided 'years_test' is used. Format: "%d/%m/%Y %H:%M"
# begin_test_date    = pd.to_datetime('18/9/2024 6:00', dayfirst=True) # Start test date, if not provided 'years_test' is used. Format: "%d/%m/%Y %H:%M"
# end_test_date      = pd.to_datetime('18/9/2024 11:45', dayfirst=True) # End test date, if not provided 'years_test' is used. Format: "%d/%m/%Y %H:%M"
...
# fc_list = [real_values, forecast]
# label_list = ['Perfect', 'Ensemble']
# fc_list = [real_values]
# label_list = ['Perfect']
profit_list = []
si_side_list = []
...
for fc in fc_list:
    ...
    for h in range(hours):
        # Considered forecast at start QH
        fc_hz = fc.loc[qh].values[:optim_hz]
        ...
        lp += pulp.lpSum([p[t] * fc_hz[t] for t in range(optim_hz)]) # Positive power = discharging
        ...
        power_solution = np.concatenate((power_solution, lp_solution))
...
for i in range(len(fc_list)):
    ax.plot(real_values.index, np.cumsum(profit_list[i])*100/np.sum(profit_list[0]), label=label_list[i])
plt.legend()
plt.xlabel('Time')
plt.ylabel('Relative cumulative profit [%]')
date1 = begin_test_date
date2 = pd.to_datetime('15/10/2024 00:00', dayfirst=True)
date3 = end_test_date
ax.set_xticks([date1, date2, date3], ['Sept. 24', 'Oct. 15', 'Nov. 11'])
plt.show()
