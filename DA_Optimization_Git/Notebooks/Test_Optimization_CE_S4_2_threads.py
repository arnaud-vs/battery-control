...
terminal_penalty_L2 = [1e-4, 3e-4]   # L2 scale (small!)
cycle_penalty_grid  = [0.0, 2.0]     # €/MWh throughput
        ...
        jobs.append(
            {
                ...
                "cycle_penalty": float(cyc),
                "label": f"L1_tp={fixed_L1_terminal_penalty}_cyc={cyc}",
            }
        )
    return jobs
...
def run_one(job: dict, *, threads_per_worker: int = 1) -> dict:
    ...
    # Unique tag to avoid file collisions across processes
    # Also filename-safe: replace '.' and '-' characters
    ...
    common_kwargs = dict(
        ...
        solver_name="gurobi",  # or "gurobi_direct" if that's your preferred interface
...
if __name__ == "__main__":
    # Strongly recommended on Windows
    mp.set_start_method("spawn", force=True)

    jobs = _make_jobs()

    # Tune these for your 10 vCPU box:
    # - Start conservative: 2 workers, 1 thread each (stable)
    # - Then consider 2 workers, 2–3 threads each
    max_workers = 2
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        ...
        for f in as_completed(futures):
            s = f.result()  # will raise the real exception if a worker fails
            summaries.append(s)
            print("Done:", s)
