...
def build_experiment_grid() -> List[ProbRunSpec]:
    ...
    # B) Copula lam_corr sweep (fixed risk)
    for r in [0.90, 0.95, 0.995]:
        runs.append(ProbRunSpec(
            scenario_method="copula",
            n_scenarios=500, lam_corr=r, lookback_days=30, base_seed=1,
            alpha=0.9, lambda_cvar=0.1,
            name=f"cop_n500_r{r}_lb30_seed1_a0.9_l0.1"
        ))
