...
TerminalPenaltyMode = Literal["none","L1","quadratic"]
...
def run_ip_rolling_ce_models(
    *,
    ...
    if save:
        ...
        fname = f"ip_rolling_ce_{models_str}_deterministic_{sources_str}{safe_tag}.parquet"
...
def _run_ip_rolling_prob_single(
    *,
    ...
    terminal_penalty: float = 0.0,
    terminal_penalty_mode: TerminalPenaltyMode = "none",
    weight_method: Literal["cdf_bins_renorm", "equal"] = "cdf_bins_renorm",
    alpha: float = 0.9,
    ...
    quantiles = _extract_quantiles_sorted(ip_prob_forecast)
    S = len(quantiles)

    if weight_method == "equal":
        weights = np.ones(S, dtype=float) / S
    elif weight_method == "cdf_bins_renorm":
        weights = _cdf_bin_weights_renorm(quantiles)
    else:
    ...
    if lambda_cvar == 0.0:
        """ add_objective_risk_neutral(   # or your add_objective_risk_neutral(...)
            m,
    ...
    for a, (ts, f_row) in enumerate(ip_prob_forecast.iterrows()):
        ...
        # Update mutable scenario prices
        for t in range(horizon_steps):
            qh = t + 1
            for s, q in enumerate(quantiles):
                ...
                m.scen_price[t, s].set_value(float(f_row[col]))
...
def run_ip_rolling_prob_models(
    *,
    ...
    if save:
        ...
        fname = f"ip_rolling_prob_{models_str}_{weight_method}_{risks_str}{safe_tag}.parquet"
