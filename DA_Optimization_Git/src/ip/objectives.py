...
def _build_terminal_penalty_expr(
    ...
    *,
    ...
    - quadratic: adds quadratic term (MIQP if binaries exist)
    """
    penalty_expr = 0.0
    ...
    elif terminal_penalty_mode == "quadratic":
...
def add_objective_certainty_equivalent(
    ...
    *,
    ...
    def _obj(mm):
        ...
        return profit - penalty_expr

    m.obj = pyo.Objective(rule=_obj, sense=pyo.maximize)


""" def add_objective_risk_neutral(
    m: pyo.ConcreteModel,
    ...
    *,
    ...
    # If you already introduced _clear_existing_objective_and_penalty in your file, use it here.
    # Otherwise, do the minimal cleanup inline:
    if hasattr(m, "obj"):
        m.del_component(m.obj)
    for comp in ["E_target", "lambda_terminal", "dev_pos", "dev_neg", "terminal_dev"]:
        if hasattr(m, comp):
    ...
    # Important: close over numpy array w (constant), not mm.S values
    def _obj(mm):
        exp_profit = 0.0
        for s in mm.S:
            ws = float(w[int(s)])
        ...
        return exp_profit - penalty_expr
...
def add_objective_mean_cvar(
    ...
    *,
    ...
    # ---- objective
    m.obj = pyo.Objective(
        expr=exp_profit - penalty_expr - float(lambda_cvar) * cvar_expr,
        sense=pyo.maximize,
    )
