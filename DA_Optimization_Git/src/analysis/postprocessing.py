...
def plot_trades_histogram(results, labels, prefixes):
    ...
    for i, r in enumerate(results):
        ...
        axs[i].hist(x=df[f'{prefixes[i]}.profit_realized_eur'], bins=range(-1000, 2000, 50))
        axs[i].grid(alpha=0.5)
        axs[i].set_title(labels[i])
        axs[i].set_yscale('log')
    fig.supxlabel('Trading profit [€]')
    fig.supylabel('Trades [-]')
    fig.suptitle('Trading profit histogram')
    plt.show()
