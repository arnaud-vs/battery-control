...
# Plotting functions
def plot_trading_decisions(results, labels, prefixes, forecast, real_prices, start, end):
    ...
    for i, r in enumerate(results):
        ...
        axs[0, i].step(real_prices.index, real_prices, where='post', label='Real imbalance price')
        axs[0, i].step(forecast.index, forecast['q0.50_qh1'], where='post', label='Forecast QH1')
        axs[0, i].legend()
...
start = pd.Timestamp(year=2023, month=6, day=1, hour=18)
end = pd.Timestamp(year=2023, month=6, day=2, hour=0)
plot_trading_decisions(results, labels, prefixes, forecast, real_prices, start, end)
